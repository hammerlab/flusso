...
var headerParser = new Parser()
  ...
  .string("analysisEnd", FCS_HEADER_8CHAR_OFFSET); // We are ignoring potential custom header fields
...
const parseText = textBuffer => {
  ...
  for (i = 1; i < values.length; i += 2) {
    let normalizedKey = values[i].replace("$", "_").replace(" ", "_");
...
const parseData = (dataBuffer, numOfParams, numOfEvents) => {
  let dataParser = new Parser().array(null, {
...
const fromFile = fileName => {
  ...
  fs.readSync(fd, dataBuffer, 0, dataLength, parseInt(text._BEGINDATA));
  let data = parseData(dataBuffer, parseInt(text._PAR), parseInt(text._TOT));
